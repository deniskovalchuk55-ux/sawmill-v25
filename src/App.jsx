...
function ShiftForm({worker,allWorkers,isOwner,onClose,onSaved,tgId,tgName,cfg}) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      vibrate('success')
      setResult({hours:h,actualH:Math.max(0,h-lb),packs:pk,earned:Math.max(0,h-lb)*(sel.rateHour||0)+pk*(sel.ratePack||0),date,timeFrom,timeTo})
      setSuccess(true)
    ...
    <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      {type!=='packs'&&<div>
        ...
        {rawH>0&&<div style={{fontSize:11,color:C.accent,marginTop:6,padding:'6px 10px',background:'rgba(56,189,248,0.06)',borderRadius:6}}>
          {hoursMode==='range'&&rangeH!==null&&<span>{fmtH(rangeH)} год </span>}
          − {lb} год обід = <b>{fmtH(actualH)} год</b> · {fmt(actualH*(sel?.rateHour||0))} грн
        </div>}
      </div>}
