...
// ПОКАЗНИКИ СВІТЛА
// ================================================================
function ElectricityView({onBack,tgId,tgName,year,month,onMonthChange,cfg}) {
  ...
  const loadData = useCallback(async(y,m)=>{
    try {
      setLoading(true)
      const [r, all] = await Promise.all([fetchElectricity(y??year,m??month), fetchElectricityAll()])
      setRows(r); setAllRows(all)
    } catch(e){} finally{ setLoading(false) }
  },[year,month])
  ...
  const monthRows = withDiff.filter(r => (r.dateShort||'').slice(0,7) === `${year}-${String(month+1).padStart(2,'0')}`)
  const totalKwh = monthRows.reduce((s,r)=>s+(r.diff||0),0)
  const totalAmt = monthRows.reduce((s,r)=>s+(r.amount||0),0)
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      vibrate('success'); setNewVal(''); setModal(null)
      loadData()
  ...
  return <div style={{minHeight:'100vh',background:C.bg,paddingBottom:40}}>
    <Header title="💡 Показники світла" sub={`${MONTHS_UA[month]} ${year}`} onBack={onBack}/>

    <div style={{padding:'16px 16px 0',display:'flex',flexDirection:'column',gap:14}}>
      <MonthPicker year={year} month={month} onChange={(y,m)=>{onMonthChange(y,m);loadData(y,m)}}/>
      ...
      <Card>
        ...
        <form onSubmit={handleSave} style={{display:'flex',flexDirection:'column',gap:10}}>
          <input type="number" inputMode="numeric" value={newVal} onChange={e=>setNewVal(e.target.value)}
            placeholder="напр. 12450 кВт" style={inp}/>
      ...
      {/* Статистика місяця */}
      {!loading&&monthRows.length>0&&<Card top={C.gold}>
        <Lbl>ПІДСУМОК МІСЯЦЯ</Lbl>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
          <div><div style={{fontSize:11,color:C.muted,marginBottom:4}}>Використано</div><div style={{fontSize:20,fontWeight:700,color:C.gold}}>{fmt(totalKwh)} кВт·год</div></div>
          {tariff>0&&<div><div style={{fontSize:11,color:C.muted,marginBottom:4}}>До оплати</div><div style={{fontSize:20,fontWeight:700,color:C.accent}}>{fmt(totalAmt)} грн</div></div>}
        </div>
        {tariff===0&&<div style={{fontSize:11,color:C.muted,marginTop:8}}>💡 Вкажи тариф в ⚙️ Налаштуваннях щоб бачити суму</div>}
      </Card>}

      {loading&&<div style={{textAlign:'center',color:C.muted,padding:20,fontSize:13}}>Завантаження...</div>}

      {/* Список записів з різницею */}
      {!loading&&<>
        <div style={{fontSize:11,color:C.muted,textTransform:'uppercase',letterSpacing:1}}>Всі записи</div>
        {allRows.length===0&&<div style={{textAlign:'center',color:C.muted,fontSize:14,padding:32}}>Ще немає жодного запису</div>}
        {withDiff.slice().reverse().map(r=>(
          <Card key={r.id}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              <span style={{fontSize:16,fontWeight:700,color:C.text}}>{r.value.toLocaleString()} кВт</span>
              <div style={{display:'flex',gap:8}}>
                <button onClick={()=>handleDelete(r)} style={{background:'transparent',border:`1px solid ${C.red}33`,color:C.red,borderRadius:6,padding:'3px 8px',fontSize:11,cursor:'pointer',fontFamily:'inherit'}}>🗑</button>
              </div>
            </div>
            {r.diff!==null&&<>
              <Row label="Використано" value={`${fmt(r.diff)} кВт·год`} color={C.gold}/>
              {tariff>0&&r.amount!==null&&<Row label="До оплати" value={`${fmt(r.amount)} грн`} color={C.accent}/>}
            </>}
            <div style={{fontSize:11,color:C.muted,marginTop:6,textAlign:'right'}}>{r.dateShort.split('-').reverse().join('.')}</div>
          </Card>
        ))}
...
function OwnerDashboard({workers,onRefresh,tgId,tgName,cfg,year,month,onMonthChange}) {
  ...
  if (view==='fuel') return <FuelView onBack={()=>setView('main')} tgId={tgId} tgName={tgName} year={year} month={month} onMonthChange={onMonthChange} isOwner={isOwner}/>
  if (view==='electricity') return <ElectricityView onBack={()=>setView('main')} tgId={tgId} tgName={tgName} year={year} month={month} onMonthChange={onMonthChange} cfg={cfg}/>
